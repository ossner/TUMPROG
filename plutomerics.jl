...
Numerical integration (a.k.a. quadrature (mostly)) is the approximation of the numerical value of a definite integral.

So basically, we are trying to calculate $\int_a^b{f(x)dx}$ as closely as possible. In numerics, we're never gonna get it 100% but there are different approaches to get it as closely as possible.
The general quadrature problem is defined as
...
begin
	width = 10/n # width of the rectangles
	points = Vector{Float64}
	points = zeros((n%2)) # If number of points odd, add 0
	i = if (n%2==0) width/2 else width end # if spokes odd first point is exactly the width
	while i <= 5
		push!(points,i,-i)
		global i = i + width
	end
	...
	sumClassical = 0
	for j in points
		midpointRect = rect(j-(width/2), 0, j+(width/2), f(j))
		classicalTrap = trap(j-(width/2), 0, j+(width/2), f)
		plot!(p1,midpointRect[1], color="teal", fill=true, legend=false, opacity=0.5)
		plot!(p2,classicalTrap[1], color="teal", fill=true, legend=false, opacity=0.5)
		global sumMidpoint = sumMidpoint + midpointRect[2]
		global sumClassical = sumClassical + classicalTrap[2]
	end
